...
>
> **Last updated:** July 2026 (revised after a fuller resume update — the startup now has a name, a history, and real technical substance to draw from). LinkedIn's algorithm and format preferences shift every few months too, so revisit Sections 6–8 periodically, and keep Section 1 current as SuperOS.dev evolves.
...
- First person, conversational — write like explaining something to a sharp peer
- When writing about SuperOS.dev, own the decisions directly ("I built," "we decided") rather than hedging
...
Stay within these 3–4 pillars. LinkedIn's ranking system rewards accounts that stick to a small, consistent topic set — that consistency is what lets it learn the account's expertise and distribute content beyond the existing network.

**Pillar 1 — Building SuperOS.dev (Founder Story & Product)**
The flagship pillar now — real, current material. Post here often.
- The pivot: why AutoCRUD's "Tables" (a no-code spreadsheet database platform) became the foundation for SuperOS.dev, and what triggered the shift toward agent-based workflow automation
...
**Pillar 2 — AI Infrastructure & LLM Productization POV (Authority)**
Opinion and analysis, informed by building SuperOS.dev day to day — reaches people outside the existing network.
...
**Pillar 3 — Scaling & Systems Engineering: Tesla, Google & Beyond (Proof)**
Deep infra credibility that pre-dates the startup — the "why trust this founder" pillar.
- *Direct experience:*
...
- *General technical writing* — not narrated as "at Tesla, we..." every time:
...
**Pillar 4 — The Investor/Operator Lens (Differentiation)**
A rare combination — deep infra experience, an active startup, and an angel-investing seat.
...
- Stay within the pillars in Section 5

**Don't:**
- Post across unrelated topics — makes the account invisible to the algorithm outside the existing network
- Pad a single post with tangential personal backstory pulled from a different pillar (e.g., a Tesla story dropped into a SuperOS.dev post) — stay on the one thing that post is actually about, unless the extra context is genuinely necessary to make the point
...
1. Identify which pillar (1–4) it maps to. If it doesn't map cleanly, reconsider it.
2. If the post references personal experience, pull a specific number, decision, or real story from Section 1 — never invent details. General technical explainers (Pillar 3's second flavor, and Pillar 2) don't need a personal anecdote, but should still show real depth, not generic filler.
3. Keep the post anchored to that single pillar/topic. Don't drag in unrelated personal context from a different pillar (a Tesla story in a SuperOS.dev post, or vice versa) unless it's genuinely necessary to the point — cut it if it's just there to sound more personal or credible.
4. Draft a 2–3 line hook that works standalone.
